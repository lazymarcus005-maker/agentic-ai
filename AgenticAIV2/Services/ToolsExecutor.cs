...
public class ToolsExecutor
{
    ...
    public async Task<Journal> ExecuteAsyncWithProgress(Plan plan, CancellationToken ct, Func<string, Task> progress)
    {
        ...
        foreach (var step in plan.Steps)
        {
            ...
            try
            {
                ...
                await progress($"exec start ดำเนินการ {step.Tool}...");
                if (step.Type == "tool")
                {
    ...
    {
        // options เฉพาะจุดนี้ (จะได้ไม่ต้องมีตัวแปร global)
        ...
        return VarToken.Replace(text, m =>
        {
            ...
            return val switch
            {
                ...
                // 🔥 กันเพี้ยน: JsonElement เอา raw JSON ออกมา ห้าม serialize ซ้ำ
                ...
                // JsonDocument ก็ raw เช่นกัน
                JsonDocument jd => jd.RootElement.GetRawText(),

                // primitives ให้เป็นข้อความธรรมดา (ถ้าคุณอยากให้เป็น JSON เสมอ บอกได้)
                bool b => b ? "true" : "false",
                byte or sbyte or short or ushort or int or uint or long or ulong
                    or float or double or decimal => Convert.ToString(val, CultureInfo.InvariantCulture) ?? "",

                DateTime dt => dt.ToString("O", CultureInfo.InvariantCulture),
                DateTimeOffset dto => dto.ToString("O", CultureInfo.InvariantCulture),
                Guid g => g.ToString(),

                // ที่เหลือค่อย serialize ทีเดียวด้วย options นี้
                _ => JsonSerializer.Serialize(val, jsonOptions)
            };
        });
    }
}
