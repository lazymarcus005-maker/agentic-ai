...
public class PlannerService
{
    ...
    {
        // treditinal way to build tools catalog, but it is slow when tools are many. So I moved it to a static method and cache the result.
        // Baseline
        //var toolsCatalog = ToolsDisc.GetTooldisc();//string.Join("\n", _tools.AllTools().Select(t => $"- {t.plugin}.{t.tool.Name}: {t.tool.Description}"));

        // mcp metadata way to build tools catalog, it is fast but less flexible. I need to make sure the format is correct in the metadata.
