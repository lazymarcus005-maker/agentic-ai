...
public class ValidatorService
{
    ...
    public Task<ValidationResult> ValidateAsync(Plan plan, CancellationToken ct = default, PolicyBlock policy = null)
    {
        ...
        foreach (var s in plan.Steps)
        {
            if (s.Type == "tool")
            {
                ...
                if (policy != null)
                {
                    if (policy.Tools.Forbidden.Contains($"{s.Plugin}.{s.Tool}"))
                    {
                        res.IsValid = false;
                        res.Errors.Add($"step {s.Id}: tool ไม่ได้รับอนุญาต {s.Plugin}.{s.Tool}");
                    }
                    else if (!policy.Tools.Allowed.Contains($"{s.Plugin}.{s.Tool}"))
                    {
                        res.IsValid = false;
                        res.Errors.Add($"step {s.Id}: tool ไม่ได้รับอนุญาต {s.Plugin}.{s.Tool}");
                    }
