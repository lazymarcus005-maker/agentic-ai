...
public class OrchestratorService
{
    ...
    public async Task<ChatResponse> ExecWithProgress(ChatRequest req, CancellationToken ct, Func<string, Task> progress, PolicyBlock policy = null)
    {
        ...
        while (replanCount <= _policy.MaxReplan)
        {
            try
            {
                await progress($"start เริ่มวางแผน (รอบที่ {replanCount + 1})...");
                plan = replanCount == 0
                    ? await _planner.CreatePlanAsync(req.Message, ct)
                    : await _planner.RePlanAsync(req.Message, feedback, ct);
                planHistories.Add(plan);

                await progress("check กำลังตรวจสอบแผน...");
                val = await _validator.ValidateAsync(plan, ct, policy);
                ...
                await progress("exec กำลังดำเนินการตามแผน...");
            ...
            {
                feedback = string.Join("; ", ex.Message);
                await progress($"แผนไม่ผ่าน: {feedback} -> วางแผนใหม่");
                replanCount++;
            }
        }
        ...
        await progress("_composer กำลังสรุปผลลัพธ์...");
        var (screen, email) = await _composer.ComposeAsync(req.Message, journal, ct);
        var runId = _store.Put(new { plan, journal, screen, email });
        var llmHistory = _llm.Current;
